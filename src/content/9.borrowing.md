...
3. **Solo una referencia mutable OR múltiples inmutables** a la vez
4. **Las referencias deben ser válidas** durante su uso (no dangling)
5. **Los slices son referencias** a porciones de colecciones
6. **`&str` es más flexible** que `&String` para parámetros
7. **Rust desreferencia automáticamente** en muchos contextos
